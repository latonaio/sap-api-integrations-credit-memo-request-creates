...
func (c *SAPAPICaller) HeaderItem(headerItem *requests.HeaderItem) {
	err := c.callCreditMemoRequestSrvAPIRequirementHeaderItem("A_CreditMemoRequest", headerItem)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callCreditMemoRequestSrvAPIRequirementHeaderItem(api string, headerItem *requests.HeaderItem) error {
	...
	headerItem.CreditMemoRequest = resBody.D.CreditMemoRequest
	return nil
}
